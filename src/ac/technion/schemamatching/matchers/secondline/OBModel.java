...
public class OBModel implements CEModel{
		...
		private void maxEntropyOneToMany() {
			...
			for (Match _m : match){
				cands.add(new CandPair(1f/mi.getMatrix().getRowCount(),_m.getCandidateTerm().getId(),_m.getTargetTerm().getId(),_m.getEffectiveness()));
			}
		}
		...
		private void removeCompetitors(CandPair select, Iterator<CandPair> candItr){
			while (candItr.hasNext()){
				...
				if (isOne2OneMatch && (check.targetId == select.targetId || check.candId == select.candId)){//1:1 match
					...
					candItr.remove();
				}else if (!isOne2OneMatch && check.candId == select.candId){//1:n match
					//remove competitor to have a correct match
					candItr.remove();
				}
			}
