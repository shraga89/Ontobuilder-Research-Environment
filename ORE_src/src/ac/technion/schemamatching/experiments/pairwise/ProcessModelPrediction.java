...
import ac.technion.schemamatching.statistics.all.AttributeNBGolden;
...
import ac.technion.schemamatching.statistics.all.MatchDistance;
	...
	public List<Statistic> runExperiment(ExperimentSchemaPair esp) {
		...
		for(ProcessModelMatcher matcher : ProcessModelMatcher.values()) {
			if (this.dutchSPIDs.contains(esp.getID()) && matcher.equals(ProcessModelMatcher.BPMatcher))
				continue;
			if (matcher.equals(ProcessModelMatcher.GEDMatcher)) continue; //Skip due to lpsolve issues
		...
		for (ProcessModelMatchStrategy strategy : ProcessModelMatchStrategy.values()) {
			if (strategy.equals(ProcessModelMatchStrategy.OptSEDOverActivityLabels)) continue; //skipped due to LPSOLVE issues
			...
			/*
			 * Added Match Distance measures by Tomer Sagi 31/12/13
			 */
			
			K2Statistic md = new MatchDistance();
			md.init(instanceDesc, miFirst, esp.getExact());
			results.add(md);
